...
def main(nu: float = NU) -> None:
    ...
    # lambda follows nu (PECMSource derives it the same way internally)
    lam = 2.0 * MU * nu / (1.0 - 2.0 * nu)
    rows = np.array([
        [x, y, s["source_x"], s["source_y"], s["depth"],
         s["omega_x_deg"], s["omega_y_deg"], s["omega_z_deg"],
         s["a_x"], s["a_y"], s["a_z"], s["pressure"], MU, lam]
    ...
    # sibling (e.g. pecm_volume_golden_nu0.32.json) so the default fixture
    # and the off-default material check coexist.
    suffix = "" if nu == NU else f"_nu{nu:g}"
    outfile = DATA / f"pecm_volume_golden{suffix}.json"
