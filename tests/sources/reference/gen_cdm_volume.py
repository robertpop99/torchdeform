#!/usr/bin/env python3
"""Generate a random-volume forward ground truth for ``CDMSource`` from the
original Nikkhoo et al. (2017) MATLAB ``CDM.m``.

The finite-CDM analogue of ``gen_pcdm_volume.py``: where ``gen_nikkhoo.m``
freezes two hand-picked orientations, this sweeps a wide random parameter space
(depth, orientation, semi-axes, opening, observation location) and freezes the
forward ENU displacement as ``../data/cdm_volume_golden.json``.

Forward only, for the same reason as the pCDM fixture: ``CDMSource`` is a plain
...
``test_cdm_source.py``. See ``gen_pcdm_volume.py`` / README for the full rationale.

Convention bridge (must match ``CDMSource``)
--------------------------------------------
``CDM.m`` takes ``omegaX/Y/Z`` in **degrees** and semi-axes ``ax/ay/az`` +
``opening`` in metres, returning surface ENU; ``CDMSource`` takes angles in
**radians**. Sampling works in degrees (for MATLAB); the JSON stores radians so
the test feeds ``CDMSource`` directly. Material: ``nu = 0.25`` (source default).

Regenerate::

    python gen_cdm_volume.py                 # needs MATLAB + vendored nikkhoo/
    python gen_cdm_volume.py --summary        # inspect committed JSON, no MATLAB
"""
...
NU = 0.25                  # CDMSource default Poisson ratio
SEED = 20240709
...
# (X Y X0 Y0 depth omegaX omegaY omegaZ ax ay az opening nu), call CDM per row,
# write ue un uv at full double precision. Absolute paths baked in (no cwd
# dependence). CDM is per-point independent, so one-row-per-point stays general.
DRIVER_M = r"""
addpath('{nikkhoo}');
inp = load('{input}');
K = size(inp, 1);
...
for i = 1:K
    r = inp(i, :);
    [ue, un, uv] = CDM(r(1), r(2), r(3), r(4), r(5), r(6), r(7), r(8), ...
...
def run_cdm(rows: np.ndarray) -> np.ndarray:
    """rows: [K,13] CDM inputs -> out: [K,3] ENU surface displacement."""
    if not (NIKKHOO / "CDM.m").is_file():
        sys.exit(f"vendored CDM.m not found in {NIKKHOO}.")
    with tempfile.TemporaryDirectory() as tmp:
        tmp = Path(tmp)
        inp, outp = tmp / "in.txt", tmp / "out.txt"
        np.savetxt(inp, rows, fmt="%.17e")
        driver = tmp / "cdm_driver.m"
...
def sample_source(rng: np.random.Generator) -> dict:
    """One physically-sane buried CDM: random orientation, semi-axes and opening.

    Depth stays well above the semi-axes so the model is fully buried; opening
    carries a random sign (tensile or closing)."""
    return dict(
        ...
        a_x=float(rng.uniform(100.0, 500.0)),
        a_y=float(rng.uniform(100.0, 500.0)),
        a_z=float(rng.uniform(100.0, 500.0)),
        opening=float(rng.choice([-1.0, 1.0]) * rng.uniform(0.1, 1.0)),
    )
...
def main(nu: float = NU) -> None:
    ...
    rows = np.array([
        [x, y, s["source_x"], s["source_y"], s["depth"],
         ...
         s["a_x"], s["a_y"], s["a_z"], s["opening"], nu]
        for s, sp in zip(sources, pts) for (x, y) in sp
    ])
    enu = run_cdm(rows).reshape(N_SOURCES, N_POINTS, 3)
    ...
    payload = {
        "_comment": (
            "Random-volume forward ground truth for CDMSource, from the original "
            "Nikkhoo (2017) MATLAB CDM.m. Generated by "
            "tests/sources/reference/gen_cdm_volume.py. Do not edit by hand."
        ),
        "poisson_ratio": nu,
        "seed": SEED,
        "n_sources": N_SOURCES,
        "n_points": N_POINTS,
        # Per-source parameters [B]. Angles in RADIANS (CDMSource units).
        ...
        "opening": [s["opening"] for s in sources],
        "x_obs": x_obs.tolist(),
        "y_obs": y_obs.tolist(),
        "u_enu": enu.tolist(),
    }
    ...
    # sibling (e.g. cdm_volume_golden_nu0.32.json) so the default fixture
    # and the off-default material check coexist.
    suffix = "" if nu == NU else f"_nu{nu:g}"
    outfile = DATA / f"cdm_volume_golden{suffix}.json"
    outfile.write_text(json.dumps(payload))
    n = N_SOURCES * N_POINTS
    print(f"wrote {outfile} ({n} points, {outfile.stat().st_size / 1024:.0f} KiB)")


def summarize(path: Path = DATA / "cdm_volume_golden.json") -> None:
    ...
    print()
    print("per-source inputs (omega in degrees, depth in km, semi-axes/opening in m)")
